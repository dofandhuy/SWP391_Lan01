...
<form action="EditQuizServlet" method="post">
    ...
    <input type="number" name="duration" value="<%=quiz.getDurationMinutes()%>" required>
    <label>Passing Score:</label>
    <input type="number" name="passingScore" value="<%=quiz.getPassingScore()%>" step="0.1" required>
    <label>Point per Question:</label>
    <input type="number" name="pointPerQuestion" value="<%=quiz.getPointPerQuestion()%>" step="0.01" required>
    <label>Max Attempts (0 = Unlimited):</label>
    <input type="number" name="maxAttempts" value="<%=quiz.getMaxAttempts() == null ? 0 : quiz.getMaxAttempts()%>" min="0">
    <label>Attempt Cooldown (hours):</label>
    <input type="number" name="attemptCooldown" value="<%=quiz.getAttemptCooldownHours() == null ? 0 : quiz.getAttemptCooldownHours()%>" min="0">
    ...
    <div style="text-align:center; margin-top:20px;">
        <button type="submit" name="status" value="Draft">💾 Update to Draft</button>
        <button type="submit" name="status" value="Pending">💾 Update to Complete</button>
    </div>
</form>
</div>
</body>
