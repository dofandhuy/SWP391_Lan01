            ...
            * {
            ...
            .review-header {
                ...
                background: linear-gradient(135deg,
                    <c:choose>
                        <c:when test="${reviewData.score >= reviewData.passingScore}">#d7f8e4, #b8ebcb</c:when>
                        <c:otherwise>#fde2e2, #f8caca</c:otherwise>
                    </c:choose>
                    );
            ...
            .score-inline {
                ...
                border: 3px solid
                    <c:choose>
                        <c:when test="${reviewData.score >= reviewData.passingScore}">#28a745</c:when>
            ...
            .fill-blank-review .correct-text {
                color: #3c763d;
                font-weight: 600;
            }
            ...
            <div class="review-header">
                <div class="header-row">
                    ...
                    <div class="score-inline">
                        <fmt:formatNumber 
                            value="${(reviewData.score / (reviewData.passingScore)) * 100}" 
                            maxFractionDigits="0"
                            />%
                    </div>
                </div>
                <p class="desc">
                    You scored 
                    <fmt:formatNumber value="${reviewData.score}" maxFractionDigits="1"/> points.
                    Passing requires 
                    <fmt:formatNumber value="${reviewData.passingScore}" maxFractionDigits="0"/> points.
                </p>
            </div>
                    ...
                    <div class="question-item">
                        <div class="question-header">
                            ...
                            <span class="points-fraction">
                                <c:choose>
                                    <c:when test="${studentAttempt != null && studentAttempt.correct}">
                                        ✓<fmt:formatNumber value="${reviewData.pointPerQuestion}" maxFractionDigits="1"/> point
                                    </c:when>
                                    <c:otherwise>✗ 0 pt</c:otherwise>
                                </c:choose>
                            </span>
                        </div>
                        ...
                        <div class="options-group">
                            <c:choose>
                                <c:when test="${question.questionType eq 'MCQ' || question.questionType eq 'TrueFalse'}">
                                    <c:forEach var="option" items="${allOptions}">
                                        ...
                                        <c:choose>
                                            ...
                                            <c:when test="${!option.correct && isSelected}">
                                ...
                                <c:when test="${question.questionType eq 'FillBlank'}">
                                    <c:set var="correctFillBlankText" value=""/>
                                    <c:forEach var="option" items="${allOptions}">
                                        <c:if test="${option.correct}">
                                            <c:set var="correctFillBlankText" value="${option.answerText}"/>
                                        </c:if>
                                    </c:forEach>
                                    <div class="fill-blank-review">
                                        ...
                                        <c:if test="${studentAttempt != null && !studentAttempt.correct}">
                                            Correct: <span class="correct-text"><c:out value="${correctFillBlankText}"/></span>
                                        </c:if>
                                    </div>
                                </c:when>
                            </c:choose>
