...
public class QuizService {
    ...
    public int createQuiz(Quiz quiz, String[] selectedQuestions,
            int easy, int medium, int hard, String category) {
        int quizID = dao.insertQuiz(quiz);
        if (quizID == -1) {
            return -1;
        }
        if (quiz.isRandom()) {
            dao.addRandomQuestions(quizID, easy, medium, hard, quiz.getCreatedBy(), category);
        }
...
public boolean assignQuizToLesson(int quizId, int lessonId) {
        return dao.updateQuizLesson(quizId, lessonId);
    }

public boolean assignQuizToModule(int quizId, int moduleId) {
    return dao.assignQuizToModule(quizId, moduleId);
}

public Quiz getQuizByModuleId(int moduleId) {
    return dao.getQuizByModuleId(moduleId);
}
}
