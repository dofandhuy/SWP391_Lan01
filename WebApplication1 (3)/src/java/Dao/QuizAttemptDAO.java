...
public class QuizAttemptDAO extends DBContext {
    ...
    public List<QuizAttemptQuestion> getStudentAnswersForAttempt(int attemptId) {
        List<QuizAttemptQuestion> list = new ArrayList<>();
        String sql = "SELECT QuestionID, AnswerIDs, IsCorrect FROM QuizAttemptQuestion WHERE AttemptID = ?";
        try (Connection conn = DBContext.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, attemptId);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                ...
                // Parse answerIDs: e.g. "1,3,4" -> [1,3,4]
                String answerStr = rs.getString("AnswerIDs");
                if (answerStr != null && !answerStr.isEmpty()) {
                    List<Integer> ids = Arrays.stream(answerStr.split(","))
                            .map(Integer::parseInt)
                            .collect(Collectors.toList());
                    qa.setSelectedAnswerIds(ids);
                }
                list.add(qa);
            }
        } catch (Exception e) {
