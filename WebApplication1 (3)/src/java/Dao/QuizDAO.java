...
public class QuizDAO extends DBContext {
    ...
    try (Connection conn = DBContext.getConnection();
        ...
        if (quiz.getReviewedBy() == null) {
            ...
            ps.setInt(10, quiz.getReviewedBy());
        }
        ...
        if (quiz.getMaxAttempts() == null) {
            ...
            ps.setInt(13, quiz.getMaxAttempts());
        }
        ...
        if (quiz.getAttemptCooldownHours() == null) {
            ...
            ps.setInt(14, quiz.getAttemptCooldownHours());
        }
...
public boolean removeQuizFromModule(int moduleId) {
   ...
   public List<Integer> getCorrectOptionIdsByQuestionId(int questionId) {
    List<Integer> correctIds = new ArrayList<>();
    String sql = "SELECT AnswerID FROM AnswerOptions WHERE QuestionID = ? AND IsCorrect = 1";
    try (Connection conn = DBContext.getConnection();
         PreparedStatement ps = conn.prepareStatement(sql)) {
        ps.setInt(1, questionId);
        ResultSet rs = ps.executeQuery();
        while (rs.next()) {
            correctIds.add(rs.getInt("AnswerID"));
        }
    } catch (Exception e) {
        e.printStackTrace();
    }
