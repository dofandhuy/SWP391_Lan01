   ...
   public List<Course> getContinueLearning(int studentId) {
    ...
    String sql = """
        SELECT 
            ...
            c.Description,
            u.FullName AS InstructorName,
            SUM(CASE WHEN sp.IsCompleted = 1 THEN 1 ELSE 0 END) AS CompletedModules,
    ...
    try (Connection con = new DBContext().getConnection(); 
        ...
        while (rs.next()) {
            ...
            // Optional: lưu thêm completed / total nếu class Course có field tương ứng
            // c.setCompletedModules(rs.getInt("CompletedModules"));
            // c.setTotalModules(rs.getInt("TotalModules"));
...
// Thêm/Sửa các phương thức này trong file Dao/CourseDAO.java

/**
 * Lấy tất cả các danh mục để đổ vào dropdown lọc.
 */
public List<Category> getAllCategories() { // Giả sử bạn có Entity/Category.java
    ...
    try (Connection conn = new DBContext().getConnection();
         ...
         ResultSet rs = ps.executeQuery()) {
        while (rs.next()) {
           Category category = new Category(0, sql);
...
/**
 * Lấy thông tin chi tiết của một khóa học để hiển thị trong modal.
 */
public Course getCourseDetailsById(int courseId) {
    Course course = null;
    String sql = "SELECT c.*, u.FullName AS InstructorName, cat.CategoryName " +
                 "FROM dbo.Courses c " +
                 "JOIN dbo.Users u ON c.InstructorID = u.UserID " +
                 "LEFT JOIN dbo.Category cat ON c.CategoryID = cat.CategoryID " +
                 "WHERE c.CourseID = ?";
    try (Connection conn = new DBContext().getConnection();
         PreparedStatement ps = conn.prepareStatement(sql)) {
        ps.setInt(1, courseId);
        ...
        if (rs.next()) {
            ...
            course.setThumbnail(rs.getString("Thumbnail"));
            course.setRejectionReason(rs.getString("RejectionReason")); // ✅ thêm dòng này
        }
    } catch (Exception e) {
        e.printStackTrace();
    }
...
/**
 * Cập nhật thông tin cơ bản của khóa học (do Admin thực hiện).
 */
// Trong file Dao/CourseDAO.java

// XÓA PHƯƠNG THỨC updateCourseStatus(int courseId, String newStatus, String adminFeedback)

// THAY THẾ PHƯƠNG THỨC updateCourseByAdmin BẰNG PHIÊN BẢN MỚI NÀY
/**
 * Cập nhật toàn bộ thông tin khóa học từ modal của Admin.
 * Bao gồm cả chi tiết (Title, Price,...) và trạng thái (Status, Feedback).
 */
public void updateCourseByAdmin(Course course) {
    String sql = "UPDATE dbo.Courses SET Title = ?, Price = ?, CategoryID = ?, Status = ?, RejectionReason = ? WHERE CourseID = ?";
    try (Connection conn = new DBContext().getConnection();
         PreparedStatement ps = conn.prepareStatement(sql)) {
        ps.setString(1, course.getTitle());
        ...
        ps.setString(5, course.getRejectionReason());
...
// === SỬA LẠI PHƯƠƠNG THỨC getCoursesForAdmin ===
public List<Course> getCoursesForAdmin(String status, int categoryId, String instructorName, String courseName, String priceSort) {
    List<Course> courseList = new ArrayList<>();
    StringBuilder sql = new StringBuilder(
        "SELECT c.CourseID, c.Title, c.Price, c.Status, u.FullName AS InstructorName, cat.CategoryName " +
        "FROM dbo.Courses c " +
        "JOIN dbo.Users u ON c.InstructorID = u.UserID " +
        "LEFT JOIN dbo.Category cat ON c.CategoryID = cat.CategoryID " + // Sửa tên bảng
        "WHERE c.Status <> 'Draft'"
    );

    if (status != null && !status.isEmpty()) sql.append(" AND c.Status = ?");
    if (categoryId > 0) sql.append(" AND c.CategoryID = ?");
    if (instructorName != null && !instructorName.isEmpty()) sql.append(" AND u.FullName LIKE ?");
    if (courseName != null && !courseName.isEmpty()) sql.append(" AND c.Title LIKE ?");

    if ("asc".equalsIgnoreCase(priceSort)) sql.append(" ORDER BY c.Price ASC");
    else if ("desc".equalsIgnoreCase(priceSort)) sql.append(" ORDER BY c.Price DESC");
    else sql.append(" ORDER BY c.CourseID ASC");
    ...
    try (Connection conn = new DBContext().getConnection();
        ...
        if (categoryId > 0) ps.setInt(paramIndex++, categoryId);
        if (instructorName != null && !instructorName.isEmpty()) ps.setString(paramIndex++, "%" + instructorName + "%");
        if (courseName != null && !courseName.isEmpty()) ps.setString(paramIndex++, "%" + courseName + "%");
        ...
        while (rs.next()) {
           ...
           course.setCourseID(rs.getInt("CourseID"));
            ...
            course.setPrice(rs.getBigDecimal("Price"));
            course.setStatus(rs.getString("Status"));
            // Giả sử bạn có setter cho các thuộc tính này trong Entity/Course.java
            course.setInstructorName(rs.getString("InstructorName")); 
            course.setCategoryName(rs.getString("CategoryName"));
