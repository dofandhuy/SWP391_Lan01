...
public boolean isTxnRefExists(String txnRef) {
    String sql = "SELECT 1 FROM PaymentTransactions WHERE TxnRef = ?";
...
public void saveTxnRef(String txnRef, int paymentId) {
    String sql = "INSERT INTO PaymentTransactions (PaymentID, TxnRef, CreatedAt) VALUES (?, ?, GETDATE())";
    try (Connection con = new DBContext().getConnection();
         PreparedStatement ps = con.prepareStatement(sql)) {
