...
public class QuestionDAO {
   ...
   // === 5️⃣ Map ResultSet sang Question ===
    ...
    q.setRejectionReason(rs.getString("RejectionReason")); // 👈 thêm dòng này
    q.setReviewedBy(rs.getInt("ReviewedBy"));              // nếu cần
    q.setReviewedAt(rs.getTimestamp("ReviewedAt") != null
        ? rs.getTimestamp("ReviewedAt").toLocalDateTime()
        : null);
...
public List<Question> searchAndSortQuestions(String keyword, String category) {
    List<Question> list = new ArrayList<>();
    ...
    // Nếu có keyword thì thêm điều kiện LIKE
    if (keyword != null && !keyword.trim().isEmpty()) {
        sql.append(" AND q.QuestionText LIKE ? ");
    }

    // Nếu có category cụ thể thì thêm điều kiện
    if (category != null && !category.trim().isEmpty()) {
        sql.append(" AND c.CategoryName = ? ");
    }

    // Sắp xếp theo tên category và question text
    sql.append(" ORDER BY c.CategoryName ASC, q.QuestionText ASC");
...
//Luồng student :
    public List<Question> getRandomQuestionsByCategory(int categoryId, int limit) throws SQLException {
        List<Question> list = new ArrayList<>();

        // ⚠️ Ghép trực tiếp số limit vào chuỗi (SQL Server không bind TOP ?)
        String sql = "SELECT TOP " + limit + " q.*, c.CategoryName "
                + "FROM QuestionBank q "
        ...
        try (Connection conn = DBContext.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            ps.setInt(1, categoryId); // chỉ còn 1 tham số
            ResultSet rs = ps.executeQuery();
            ...
            while (rs.next()) {
                ...
                List<AnswerOption> options = getAnswerOptionsByQuestionId(q.getQuestionID());
            ...
            e.printStackTrace();
            System.err.println("❌ Lỗi trong getRandomQuestionsByCategory: " + e.getMessage());
        }
        return list;
    }
    
    public List<AnswerOption> getOptionsByQuestion(int questionID) throws SQLException {
        List<AnswerOption> list = new ArrayList<>();
        String sql = "SELECT * FROM AnswerOptions WHERE QuestionID = ?";
        try (Connection conn = DBContext.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, questionID);
            ...
            while (rs.next()) {
                ...
                opt.setAnswerID(rs.getInt("OptionID"));
                opt.setQuestionID(rs.getInt("QuestionID"));
                opt.setAnswerText(rs.getString("OptionText"));
                opt.setCorrect(rs.getBoolean("IsCorrect"));
                list.add(opt);
            }
        }
        return list;
...
//    // ✅ Lấy danh sách câu trả lời của 1 câu hỏi (chuyển từ QuizDAO sang)
...
//    }
    ...
    // ✅ Lấy 1 câu hỏi kèm các đáp án (dùng phương thức nội bộ)
    public Question getQuestionWithAnswersById(int questionId) {
        Question q = null;
        String sql = """
            SELECT q.*, c.CategoryName, u.FullName AS CreatedByName
            FROM QuestionBank q
            LEFT JOIN Category c ON q.CategoryID = c.CategoryID
        ...
        try (Connection conn = DBContext.getConnection();
            ...
            if (rs.next()) {
                ...
                // Sửa đổi quan trọng: Gọi phương thức trong cùng class
                q.setOptions(this.getAnswerOptionsByQuestionId(questionId));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    // ✅ Lấy danh sách câu hỏi có lọc/pagination
    public List<Question> getQuestionsForAdmin(String searchText, int categoryId, String createdBy, String type, String status, int page, int pageSize) {
        List<Question> list = new ArrayList<>();
        StringBuilder sql = new StringBuilder("""
            SELECT q.QuestionID, q.QuestionText, q.QuestionType, q.Status, q.CreatedBy,
                   ISNULL(c.CategoryName, 'N/A') AS CategoryName, u.FullName AS CreatedByName
            FROM QuestionBank q
            LEFT JOIN Category c ON q.CategoryID = c.CategoryID
            LEFT JOIN Users u ON q.CreatedBy = u.UserID
            WHERE 1=1
        ...
        if (searchText != null && !searchText.trim().isEmpty()) { sql.append(" AND q.QuestionText LIKE ?"); params.add("%" + searchText + "%"); }
        if (categoryId > 0) { sql.append(" AND q.CategoryID = ?"); params.add(categoryId); }
        if (createdBy != null && !createdBy.trim().isEmpty()) { sql.append(" AND u.FullName LIKE ?"); params.add("%" + createdBy + "%"); }
        if (type != null && !type.isEmpty()) { sql.append(" AND q.QuestionType = ?"); params.add(type); }
        if (status != null && !status.isEmpty()) { sql.append(" AND q.Status = ?"); params.add(status); }
        sql.append(" ORDER BY q.QuestionID DESC OFFSET ? ROWS FETCH NEXT ? ROWS ONLY");
    ...
    // ✅ Đếm câu hỏi
    public int countQuestionsForAdmin(String searchText, int categoryId, String createdBy, String type, String status) {
        StringBuilder sql = new StringBuilder("SELECT COUNT(*) FROM QuestionBank q LEFT JOIN Users u ON q.CreatedBy = u.UserID WHERE 1=1");
        List<Object> params = new ArrayList<>();
        if (searchText != null && !searchText.trim().isEmpty()) { sql.append(" AND q.QuestionText LIKE ?"); params.add("%" + searchText + "%"); }
        if (categoryId > 0) { sql.append(" AND q.CategoryID = ?"); params.add(categoryId); }
        if (createdBy != null && !createdBy.trim().isEmpty()) { sql.append(" AND u.FullName LIKE ?"); params.add("%" + createdBy + "%"); }
    ...
    // ✅ Admin duyệt câu hỏi
    public void updateQuestionStatusByAdmin(int questionId, String status, String rejectionReason, int adminId) {
        String sql = "UPDATE QuestionBank SET Status=?, RejectionReason=?, ReviewedBy=?, ReviewedAt=GETDATE() WHERE QuestionID=?";
        try (Connection conn = DBContext.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
