...
public class SubmitQuizServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            double totalScore = 0.0; // Sẽ được tính sau vòng lặp
            ...
            for (String paramName : studentAnswersMap.keySet()) {
                ...
                if (paramName.startsWith("q")) {
                    try {
                        ...
                        if (isStudentCorrect) {
                            correctCount++;
                            // totalScore += pointPerQuestion; // <-- REMOVED: Không cộng điểm ở đây
                        } else {
                            incorrectCount++;
                        }
            ...
            //tính progress sau khi xong quiz cuối detail
         
            int moduleId = moduleDAO.getModuleIdByQuizId(quizId);
            int enrollmentId = enrollmentDAO.getEnrollmentIdByStudentId(currentUser.getUserID());
            if (totalScore >= quiz.getPassingScore()) {
                progressDAO.insertProgress(enrollmentId, moduleId);
            }
