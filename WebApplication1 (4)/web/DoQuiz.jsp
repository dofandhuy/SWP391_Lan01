...
<head>
    ...
    <style>
        ...
        .sidebar h3 { text-align: center; font-size: 1.1rem; color: #222; margin-bottom: 15px; }
        .question-nav {
            display: grid;
        ...
        .question-btn {
            width: 45px; height: 45px; border: none; border-radius: 10px;
            font-weight: bold; cursor: pointer; transition: all 0.2s;
        }
        .question-btn.unanswered { background-color: #e9ecef; color: #333; }
        .question-btn.answered { background-color: #28a745; color: white; }
        .question-btn.active { background-color: #007bff; color: white; transform: scale(1.05); }
        ...
        .bottom-controls { display: flex; justify-content: space-between; gap: 8px; }
        .nav-btn, .submit-btn {
            flex: 1; background-color: #6c757d; color: white; padding: 10px 0;
            border: none; border-radius: 6px; font-size: 0.9rem; cursor: pointer; transition: 0.2s;
        }
        .nav-btn:hover, .submit-btn:hover { background-color: #5a6268; }
        ...
        .quiz-header { display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid #eee; padding-bottom: 10px; margin-bottom: 20px; }
        .quiz-header h2 { font-size: 1.4rem; color: #333; }
        .question-card {
            display: none; background: #fff; border: 1px solid #ddd; border-radius: 10px;
            padding: 25px; box-shadow: 0 2px 6px rgba(0,0,0,0.05);
            animation: fadeIn 0.3s ease-in-out;
        }
        .question-card.active { display: block; }
        .question-text { font-size: 1.1rem; font-weight: 500; color: #333; margin-bottom: 15px; }
        .option-item { margin-bottom: 10px; display: flex; align-items: flex-start; }
        .option-item input[type="radio"] { margin-right: 10px; margin-top: 5px; }
        .option-item label { cursor: pointer; color: #444; }
        .fill-blank-input { width: 80%; padding: 8px 12px; border: 1px solid #ccc; border-radius: 5px; font-size: 1rem; }
        ...
        .modal-content p { font-size: 1rem; margin-bottom: 20px; }
        .close-btn {
            padding: 8px 16px; background-color: #007bff; color: #fff; border: none; border-radius: 6px; cursor: pointer; font-size: 0.9rem;
        }
...
<script>
    ...
    function nextQuestion() { if (currentIndex < totalQuestions - 1) showQuestion(currentIndex + 1); }
    ...
    form.addEventListener('submit', function(e) {
        ...
        questions.forEach((qCard, index) => {
            ...
            if (!answered) {
                allAnswered = false;
                document.getElementById('btn-' + index).classList.add('unanswered');
                document.getElementById('btn-' + index).classList.remove('answered');
        ...
        if (!allAnswered) {
            ...
            if(firstUnanswered !== null) showQuestion(firstUnanswered);
        }
    });
